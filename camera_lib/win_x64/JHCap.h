...
Return: returns API_OK on success, otherwise returns API_ERROR.
Note:  These properties can be set by usbVideo or programming api
       "image_format", 
	   "resolution_index"
	   "enable_roi"
	   "roi_offset_x"
...
/*==============================================================
Name: CameraQueryImageLV
Description: Get a image from camera for Labview, 32bit(BGRA) pixel
Parameters: device_id, the camera index
            *imgbuf, return the image data
            *length, return the pixel number(buffer size in int)
            option, the format of the image, or trigger/sync option
Return: returns API_OK on success, otherwise returns API_ERROR.
Note:
--------------------------------------------------------------*/

extern "C" DLL_EXPORT API_STATUS __stdcall CameraQueryImageLV(int device_id,unsigned int *imgbuf, int *length, int option);
